            ...
            ud.chosenWargear.forEach((w) => {
                ...
                if (wargearClasses.length == 0) {
                    results.wargearClassPoints.set(
                        'None',
                        results.wargearClassPoints.get('None') ?? 0 + ud.points
                    );
                } else {
                    wargearClasses.forEach((wc) => {
                        results.wargearClassPoints.set(
                            wc.recommendationText,
                            results.wargearClassPoints.get(wc.recommendationText) ?? 0 + ud.points
                        );
                    });
                }
            ...
            ud.modelDatasheets.forEach((md) => {
                ...
                if (unitClass  == null) {
                    results.unitClassPoints.set(
                        'None',
                        results.unitClassPoints.get('None') ?? 0 + md.count
                    );
                } else {
                    results.unitClassPoints.set(
                        unitClass.description,
                        results.unitClassPoints.get(unitClass.description) ?? 0 + md.count
                    );
                }
